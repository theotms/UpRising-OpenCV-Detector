...
std::map<int, MovementCommand> AIHandler::predictMovements(const WorldState& world) {
    ...
    try {
        ...
        for (size_t i = 0; i < bot_order.size(); ++i) {
            ...
            // Direct model outputs: left/right motors in [-1, 1]
            float left_motor  = actions_data[i * 2];     // First value = left wheel
            float right_motor = actions_data[i * 2 + 1]; // Second value = right wheel
            ...
            // --- DEBUGGING PRINTS ---
            std::cout << "OUTPUT - AI Action:  left:" << left_motor
                      << ", right:" << right_motor << std::endl;
